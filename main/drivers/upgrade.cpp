...
bool UpgradeHandler::partitionValid() {
    ...
    if (strcmp(current_description->project_name, new_description.project_name)) {
        ESP_LOGW(
            TAG, "Invalid image name %s, should be %s", new_description.project_name, current_description->project_name
        );
        return false;
    }
    return true;
...
void UpgradeHandler::fail() {
    image_size_ = 0;
    if (update_handle_) esp_ota_end(update_handle_);
};
